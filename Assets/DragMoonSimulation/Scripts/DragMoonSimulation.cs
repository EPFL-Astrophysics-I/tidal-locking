...
public class DragMoonSimulation : Simulation
{
    ...
    private LineRenderer body2Orbit;
    ...
    void Start()
    {
        if (earthPrefab && moonPrefab)
        {
            ...
            body2Orbit = Instantiate(orbitPrefab, transform).GetComponent<LineRenderer>();
            body2Orbit.positionCount = 0;
        }
    ...
    private void DrawOrbit() {
        ...
        if (body2Orbit)
        {
            ...
            for (int i = 0; i < numSteps; i++)
            {
                ...
                float r2 = -mass1 / totalMass * r;
                positions[i] = r2 * (Mathf.Cos(theta) * xHat + Mathf.Sin(theta) * yHat);
            }

            body2Orbit.positionCount = numSteps;
            body2Orbit.SetPositions(positions);
            body2Orbit.loop = true;
        }
    }
